...
class ProposalController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.proposal.create', compact('sekolahList', 'statusList', 'kecamatanList'));
    }    
    ...
    public function store(Request $request)
{
    ...
    return redirect()->route('admin.proposal.index')->with('success', 'Proposal berhasil dibuat.');
}
    ...
    public function edit($id)
    {
        ...
        return view('admin.proposal.edit', compact('proposal', 'sekolahList', 'statusList', 'kecamatanList'));
    }
    ...
    public function sendWhatsAppMessage($proposalId)
{
  ...
  if (curl_errno($curl)) {
      ...
      return redirect()->route('admin.proposal.index')->with('error', 'Gagal mengirim pesan: ' . $error_msg);
  }
  curl_close($curl);
  ...
  if ($response) {
      ...
      if (isset($responseDecoded['status']) && $responseDecoded['status'] == 'success') {
          return redirect()->route('admin.proposal.index')->with('success', 'Pesan WhatsApp berhasil dikirim.');
      } else {
          // Jika respon tidak sesuai dengan yang diharapkan
          return redirect()->route('admin.proposal.index')->with('error', 'Gagal mengirim pesan WhatsApp.');
      }
  }

  // Redirect default jika tidak ada respon atau respon tidak dapat di-decode
  return redirect()->route('admin.proposal.index')->with('error', 'Tidak ada respon dari API.');
}
